...
@pytest.fixture()
def mock_table() -> Generator[MagicMock]:
    """Mock DynamoDB table"""
    with patch("functions.events.create_event.get_table") as mock_get_table, patch(
        "functions.events.delete_event.get_table"
    ) as mock_delete_table, patch(
        "functions.events.get_event.get_table"
    ) as mock_get_table_get, patch(
        "functions.events.update_event.get_table"
    ) as mock_update_table, patch(
        "functions.events.get_events_by_time_range.get_table"
    ) as mock_range_table:
...
class TestCreateEvent:
    def test_create_event_success(self, mock_table, sample_event_data):
        ...
        event = {"body": json.dumps(sample_event_data)}
    ...
    def test_create_event_validation_error(self, mock_table):
        ...
        event = {"body": json.dumps({"id": "123"})}  # Missing required fields
...
class TestUpdateEvent:
    ...
    def test_update_event_invalid_json(self, mock_table):
        ...
        event = {"pathParameters": {"id": "123"}, "body": "invalid json"}
...
class TestGetEventsByTimeRange:
    def test_get_events_by_time_range_success(self, mock_table, sample_event_data):
        ...
        mock_table.scan.return_value = {"Items": [sample_event_data], "Count": 1}
    ...
    def test_get_events_by_time_range_with_pagination(
        self, mock_table, sample_event_data
    ):
        """Test retrieval with pagination"""
        # First call returns items with LastEvaluatedKey
        mock_table.scan.side_effect = [
            {"Items": [sample_event_data], "LastEvaluatedKey": {"id": "123"}},
            {"Items": [sample_event_data], "Count": 1},
        ]
    ...
    def test_get_events_by_time_range_missing_params(self, mock_table):
        ...
        event = {"queryStringParameters": {"start_time": "2025-01-01"}}
